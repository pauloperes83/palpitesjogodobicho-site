...
# O TOKEN deve estar configurado no 'git remote', não aqui no código.
...
def salvar_e_push():
    ...
    # Template HTML BASE (IDÊNTICO AO QUE VOCÊ MANDOU)
...
<style>
    ...
    h1 {{ font-size: 2.2rem; color: #222; text-align: center; }}
    ...
    .footer-social svg {{ width: 30px; height: 30px; fill: #ffffff; margin: 0 10px; }}
    .footer-links a {{ color: #d8dcec; text-decoration: none; margin: 0 10px; font-size: 0.95rem; }}
</style></head><body>
<header><div class="container"><a href="index.html" class="logo"><img src="images/logo-palpites.png"></a></div></header>
<nav><div class="container"><a href="index.html">Início</a><a href="palpite-do-dia.html">Palpite do Dia</a>
...
<a href="puxadas-do-bicho.html">Puxadas</a><a href="milhares-viciadas.html">Milhares</a><a href="https://resultadosdojogo.com/" target="_blank" style="color: #f6c945;">Resultados</a></div></nav>
<section class="section"><div class="container">
    ...
    <h1>{kw}</h1>
    <p>Confira o <strong>{kw}</strong> para hoje. Acompanhe nossa análise baseada no <a href="https://resultadosdojogo.com/" class="links-seo" target="_blank">resultado do jogo do bicho de hoje rio</a>.</p>
    <div style="text-align: center;"><a href="{btn_link}" class="btn-apostar">{btn_text}</a></div>
    {palpites_txt}
    <h2>O Segredo do {kw}</h2><p>Utilizamos estatísticas de puxadas para definir os bichos mais fortes do dia.</p>
    {grid_bichos}
    <hr><h2>Palpite Federal de Hoje</h2><p>Válido para quartas e sábados às 20h.</p>
</div></section>
<footer class="site-footer"><div class="container">
    <div class="footer-title">Palpites do Jogo do Bicho</div>
    <p style="font-size:0.85rem; color:#888ea1; max-width:700px; margin:20px auto;">Esclarecemos que não temos vínculo com o serviço ou pessoas que operam o Jogo do Bicho.</p>
    ...
    <p style="font-size:0.8rem; color:#565d6d; margin-top:20px;">© 2026 Portal de Palpites. Todos os direitos reservados.</p>
</div></footer></body></html>'''

    # Grava as páginas
    with open("/var/www/meusite/palpite-do-bicho-rj.html", 'w', encoding='utf-8') as f:
        f.write(build_page(f"Palpite Rio {hoje}", f"Palpite do dia do Jogo do Bicho de hoje Rio {hoje}", "https://app.aguiaprime119000.com/pr/y8X6LEBU", "🎰 APOSTAR NO RIO"))
    ...
    subprocess.run(["git", "commit", "-m", f"Restore Design {hoje}"])
