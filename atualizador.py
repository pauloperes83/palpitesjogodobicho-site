...
def atualizar_nacional_automatizado():
    hoje = datetime.now()
    soma_dia = hoje.day + hoje.month
    if soma_dia > 25:
        soma_dia = soma_dia - 25
    
    bicho_base_nome = ""
    for nome, dados in bichos_oficiais.items():
        if int(dados["gr"]) == soma_dia:
            bicho_base_nome = nome
            break
    if not bicho_base_nome:
        bicho_base_nome = "Jacaré"

    puxadas_nomes = bichos_oficiais[bicho_base_nome]["puxa"].split(", ")
    selecionados = [bicho_base_nome] + puxadas_nomes[:4]
    
    html_palpites = ""
    for nome in selecionados:
        if nome in bichos_oficiais:
            bicho = bichos_oficiais[nome]
            milhar = f"{random.randint(10, 99)}{random.choice(bicho['dz'])}"
            centena = milhar[1:]
            dezena = milhar[2:]
            html_palpites += f"""
        <div class="palpite-box">
            <div class="bicho-title"><h3>{bicho['gr']} - {nome.upper()} {bicho['e']}</h3></div>
            <div class="numbers-grid">
                <div class="num-card"><label>Milhar</label><span>{milhar}</span></div>
                <div class="num-card"><label>Centena</label><span>{centena}</span></div>
                <div class="num-card"><label>Dezena</label><span>{dezena}</span></div>
            </div>
        </div>"""
    
    arquivo_path = "/var/www/meusite/palpite-do-bicho-nacional.html"
    if os.path.exists(arquivo_path):
        with open(arquivo_path, "r", encoding="utf-8") as f:
            conteudo = f.read()
        
        # Substitui os palpites antigos buscando o título "Como Ganhar"
        novo_html = re.sub(r'<div class="palpite-box">.*?<h2>Como Ganhar', html_palpites + '\n        <h2>Como Ganhar', conteudo, flags=re.DOTALL)
        
        # Atualiza a data no título
        data_hoje = hoje.strftime("%d/%m/%Y")
        novo_html = re.sub(r'Nacional de Hoje \d{2}/\d{2}/\d{4}', f'Nacional de Hoje {data_hoje}', novo_html)
        
        with open(arquivo_path, "w", encoding="utf-8") as f:
            f.write(novo_html)
        print(f"Sucesso: {arquivo_path} atualizado!")
    else:
        print(f"Erro: {arquivo_path} nao encontrado!")
...
def executar():
    # Chama a função da Nacional antes de gerar os outros arquivos
    atualizar_nacional_automatizado()
    ...
    <p>Consulte {l_pal}, {l_mil} e {l_pux} para aumentar suas chances.</p>
    {grid_bichos}'''
    ...
    art_look = f'''<p>Procurando pelo melhor <strong>{kw_look}</strong>? Foco nas tendências exclusivas de Goiás.</p>
